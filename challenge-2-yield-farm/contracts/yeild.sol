...
contract YieldFarm is ReentrancyGuard, Ownable {
    ...
    function earned(address _user) public view returns (uint256) {
        ...
        uint256 boostedReward = ((user.amount * (rewardPerTokenValue - user.rewardDebt)) / 1e18);
        uint256 boostMultiplier = calculateBoostMultiplier(_user);
        return user.pendingRewards + ((boostedReward * boostMultiplier) / 100);
    }
    ...
    function calculateBoostMultiplier(
        ...
        if (stakingDuration >= BOOST_THRESHOLD_3) {
            return 200; // 2x multiplier for 90+ days
        } else if (stakingDuration >= BOOST_THRESHOLD_2) {
            return 150; // 1.5x multiplier for 30+ days
        } else if (stakingDuration >= BOOST_THRESHOLD_1) {
            return 125; // 1.25x multiplier for 7+ days
        }
        return 100; // 1x multiplier for less than 7 days
