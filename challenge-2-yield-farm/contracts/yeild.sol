...
contract YieldFarm is ReentrancyGuard, Ownable {
    ...
    function earned(address _user) public view returns (uint256) {
        ...
        // 3. Return total pending rewards
        UserInfo storage user = userInfo[_user];
        uint256 baseReward = ((user.amount * rewardPerToken()) / 1e18) - user.rewardDebt;
        uint256 boostedReward = (baseReward * calculateBoostMultiplier(_user)) / 100;
    ...
    function calculateBoostMultiplier(
        ...
        if (duration >= BOOST_THRESHOLD_3) return 200; // 2.0x
        if (duration >= BOOST_THRESHOLD_2) return 150; // 1.5x
        if (duration >= BOOST_THRESHOLD_1) return 125; // 1.25x
        return 100; // 1.0x
    }
